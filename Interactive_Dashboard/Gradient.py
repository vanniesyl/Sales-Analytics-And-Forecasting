...
if fl is not None:
    ...
    os.chdir(r"D:\Projects\Interactive_Dashboard")
...
try:
    ...
    filtered_df['DayOfWeek'] = filtered_df['Order Date'].dt.dayofweek
    filtered_df['Quarter'] = filtered_df['Order Date'].dt.quarter
...
X = filtered_df[['Order Date Ordinal', 'Month', 'Year', 'DayOfWeek', 'Quarter']]
y = filtered_df['Sales']
...
future_dates_df = pd.DataFrame({
    ...
    'DayOfWeek': future_dates.map(lambda x: x.dayofweek),
    'Quarter': future_dates.map(lambda x: x.quarter)
})

# Predict sales for future dates
future_sales = model.predict(future_dates_df[['Order Date Ordinal', 'Month', 'Year', 'DayOfWeek', 'Quarter']])
...
linechart_combined = combined_df.groupby(['month_year', 'Type'])['Sales'].sum().reset_index()
...
fig_combined = px.line(linechart_combined, x="month_year", y="Sales", color='Type', labels={"Sales": "Amount"}, 
...
with st.expander("View Data of Predicted TimeSeries:"):
